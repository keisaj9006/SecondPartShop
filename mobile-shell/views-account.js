...
const account=async(payload)=>{
 ...
 html.push("<div class=\"segmented account-mode-switch\" style=\"margin-top:14px\"><button id=\"mode-buying\" class=\""+(mode==="buying"?"active":"")+"\" type=\"button\">Buyer</button><button id=\"mode-selling\" class=\""+(mode==="selling"?"active":"")+"\" type=\"button\">Seller</button></div>");

 if(mode==="buying"){
  html.push("<section class=\"account-grid\" style=\"margin-top:14px\"><button class=\"account-tile\" id=\"account-saved\" type=\"button\"><strong>Saved parts</strong><small>Parts you want to come back to.</small></button><button class=\"account-tile\" id=\"account-saved-searches\" type=\"button\"><strong>Saved searches</strong><small>Reuse a part, vehicle and filter combination.</small></button><button class=\"account-tile\" id=\"account-recently-viewed\" type=\"button\"><strong>Recently viewed</strong><small>Parts you opened recently.</small></button><button class=\"account-tile\" id=\"account-requests\" type=\"button\"><strong>Part requests</strong><small>Ask the marketplace for a part you could not find.</small></button><button class=\"account-tile\" id=\"account-fitting\" type=\"button\"><strong>Buy + Fit requests</strong><small>Track labour quotes and arrange fitting with a garage.</small></button><button class=\"account-tile\" id=\"account-garages\" type=\"button\"><strong>Buy + Fit garages</strong><small>Find workshops that fit customer-supplied recycled parts.</small></button><button class=\"account-tile\" id=\"account-notifications\" type=\"button\"><strong>Notifications</strong><small>"+C.escapeHtml(C.state.unreadNotifications)+" unread marketplace update(s).</small></button><button class=\"account-tile\" id=\"account-garage\" type=\"button\"><strong>Garage</strong><small>Your saved vehicles and compatibility filters.</small></button><button class=\"account-tile\" id=\"account-orders\" type=\"button\"><strong>Purchases</strong><small>Payment, delivery and buyer protection.</small></button><button class=\"account-tile\" id=\"account-reviews\" type=\"button\"><strong>Reviews</strong><small>Rate completed sellers and see review opportunities.</small></button><button class=\"account-tile\" id=\"account-cases\" type=\"button\"><strong>Returns & cases</strong><small>Cancellations, returns, disputes and private photo evidence.</small></button><button class=\"account-tile\" id=\"account-inbox\" type=\"button\"><strong>Part questions</strong><small>Questions with sellers and buyers.</small></button></section>");
  ...
  if(!seller){
   ...
   html.push("<section class=\"account-grid\" style=\"margin-top:14px\"><button class=\"account-tile\" id=\"account-seller\" type=\"button\"><strong>Seller dashboard</strong><small>Readiness, sales, payouts, fulfilment and cases.</small></button><button class=\"account-tile\" id=\"account-inventory\" type=\"button\"><strong>Inventory</strong><small>Create, edit and photograph listings.</small></button><button class=\"account-tile\" id=\"account-imports\" type=\"button\"><strong>Bulk CSV import</strong><small>Preview inventory files and create draft listings without leaving the app.</small></button><button class=\"account-tile\" id=\"account-seller-requests\" type=\"button\"><strong>Buyer requests</strong><small>Open privacy-safe demand leads from buyers who could not find a part.</small></button><button class=\"account-tile\" id=\"account-donors\" type=\"button\"><strong>Donor vehicles</strong><small>Reuse source-vehicle details across many listings.</small></button><button class=\"account-tile\" id=\"account-seller-profile\" type=\"button\"><strong>Seller profile</strong><small>Edit your public seller identity and location.</small></button>"+(seller.sellerType==="business"?"<button class=\"account-tile\" id=\"account-seller-verification\" type=\"button\"><strong>Business verification</strong><small>"+(seller.verified?"Verified business":"Verification status and request")+".</small></button>":"")+"<button class=\"account-tile\" id=\"account-reviews\" type=\"button\"><strong>Reviews</strong><small>Rate completed buyers and see review opportunities.</small></button><button class=\"account-tile\" id=\"account-inbox\" type=\"button\"><strong>Buyer questions</strong><small>Pre-purchase messages about your parts.</small></button><button class=\"account-tile\" id=\"account-notifications\" type=\"button\"><strong>Seller notifications</strong><small>"+C.escapeHtml(C.state.unreadNotifications)+" unread update(s).</small></button><button class=\"account-tile\" id=\"account-cases\" type=\"button\"><strong>Returns & cases</strong><small>Transaction problems and evidence.</small></button><button class=\"account-tile\" id=\"account-new-listing\" type=\"button\"><strong>Create listing</strong><small>Add a part directly from the app.</small></button></section>");
  }
 }
 ...
 const savedSearches=document.getElementById("account-saved-searches");if(savedSearches)savedSearches.addEventListener("click",()=>UI.route("savedSearches"));
 const recentlyViewed=document.getElementById("account-recently-viewed");if(recentlyViewed)recentlyViewed.addEventListener("click",()=>UI.route("recentlyViewed"));
 const requests=document.getElementById("account-requests");if(requests)requests.addEventListener("click",()=>UI.route("requests"));
 ...
 const garages=document.getElementById("account-garages");if(garages)garages.addEventListener("click",()=>UI.route("garages"));
 ...
 const imports=document.getElementById("account-imports");if(imports)imports.addEventListener("click",()=>UI.route("sellerImports"));
 const sellerRequests=document.getElementById("account-seller-requests");if(sellerRequests)sellerRequests.addEventListener("click",()=>UI.route("sellerRequests"));
 const donors=document.getElementById("account-donors");if(donors)donors.addEventListener("click",()=>UI.route("sellerDonors"));
 ...
 document.getElementById("seller-setup-form").addEventListener("submit",async event=>{
  ...
  try{
   await C.api("/seller/profile",{method:"POST",auth:true,body:{
    ...
    businessKind:document.getElementById("seller-business-kind").value||null,
...
const sellerProfile=async()=>{
 ...
 UI.app.innerHTML="<button class=\"back\" id=\"seller-profile-back\" type=\"button\">‹ Back to Selling</button><section class=\"auth-card\"><p class=\"eyebrow\">Seller settings</p><h1 style=\"font-size:28px;margin:5px 0\">Edit seller profile</h1><p class=\"subtle\">These details are public and help buyers understand who they are buying from.</p>"+(seller.verified?"<div class=\"status warning\" style=\"margin-top:12px\"><strong>Verified business</strong><div style=\"margin-top:3px\">Changing seller type, business category, seller name, town/city or postcode removes the verified badge until the updated identity is reviewed again.</div></div>":"")+"<form id=\"seller-profile-form\" class=\"form-grid\" style=\"margin-top:16px\"><label class=\"label\">Seller type<select id=\"sp-type\" class=\"select\"><option value=\"private\">Private seller</option><option value=\"business\">Business seller</option></select></label><label class=\"label\">Business category<select id=\"sp-business-kind\" class=\"select\"><option value=\"\">Choose business type</option><option value=\"breaker\">Vehicle breaker</option><option value=\"garage\">Garage / workshop</option><option value=\"atf\">ATF / vehicle dismantler</option><option value=\"parts_business\">Parts business / retailer</option><option value=\"other\">Other automotive business</option></select></label><label class=\"label\">Seller / business name<input id=\"sp-name\" class=\"input\" minlength=\"2\" maxlength=\"140\" required value=\""+C.escapeHtml(seller.businessName||"")+"\"></label><label class=\"label\">Town or city<input id=\"sp-location\" class=\"input\" maxlength=\"140\" required value=\""+C.escapeHtml(seller.location||"")+"\"></label><label class=\"label\">Postcode <span class=\"subtle\">(optional)</span><input id=\"sp-postcode\" class=\"input\" maxlength=\"20\" value=\""+C.escapeHtml(seller.postcode||"")+"\"></label><label class=\"label\">About the seller<textarea id=\"sp-description\" class=\"textarea\" minlength=\"20\" maxlength=\"2000\" required>"+C.escapeHtml(seller.description||"")+"</textarea></label><div id=\"seller-profile-status\"></div><button id=\"seller-profile-save\" class=\"primary wide\" type=\"submit\">Save seller profile</button></form></section>";
 document.getElementById("sp-type").value=seller.sellerType||"private";
 document.getElementById("sp-business-kind").value=seller.businessKind||"";
 const spType=document.getElementById("sp-type");const spBusinessKind=document.getElementById("sp-business-kind");const syncSpKind=()=>{spBusinessKind.disabled=spType.value!=="business";spBusinessKind.required=spType.value==="business";if(spBusinessKind.disabled)spBusinessKind.value="";};spType.addEventListener("change",syncSpKind);syncSpKind();
 ...
 document.getElementById("seller-profile-form").addEventListener("submit",async event=>{
  ...
  try{
   const result=await C.api("/seller/profile",{method:"PATCH",auth:true,body:{
    ...
    businessKind:document.getElementById("sp-business-kind").value||null,
...
const sellerVerification=async()=>{
 ...
 if(result.sellerType!=="business"){
  ...
  html.push("<section class=\"card\" style=\"margin-top:12px\"><div class=\"status warning\"><strong>Review pending</strong><div style=\"margin-top:3px\">Requested "+C.escapeHtml(C.dateOnly(request.requestedAt))+". No verified badge is shown until the request is approved.</div></div><p class=\"subtle\" style=\"margin-top:10px\"><strong>Legal name:</strong> "+C.escapeHtml(request.legalBusinessName||"")+(request.businessReference?"<br><strong>Reference:</strong> "+C.escapeHtml(request.businessReference):"")+(request.referenceUrl?"<br><strong>Public URL:</strong> "+C.escapeHtml(request.referenceUrl):"")+"</p>"+(request.message?"<p class=\"subtle\" style=\"margin-top:10px\">Your note: "+C.escapeHtml(request.message)+"</p>":"")+"</section>");
 }else{
  html.push("<section class=\"auth-card\" style=\"margin-top:12px\">"+(request?.status==="rejected"?"<div class=\"status error\"><strong>Previous request was not approved</strong>"+(request.reviewNote?"<div style=\"margin-top:3px\">"+C.escapeHtml(request.reviewNote)+"</div>":"")+"</div>":"")+"<h3 style=\"margin:12px 0 4px\">Request manual verification</h3><p class=\"subtle\">Verification is a trust badge, not a requirement for Stripe payouts. SecondPart reviews the business identity separately.</p><form id=\"seller-verification-form\" class=\"form-grid\" style=\"margin-top:14px\"><label class=\"label\">Legal / registered business name<input id=\"verification-legal-name\" class=\"input\" minlength=\"2\" maxlength=\"180\" required value=\""+C.escapeHtml(C.state.me?.seller?.businessName||"")+"\"></label><label class=\"label\">Business / licensing reference <span class=\"subtle\">(if available)</span><input id=\"verification-reference\" class=\"input\" maxlength=\"180\" placeholder=\"Company, ATF, licence or trade reference\"></label><label class=\"label\">Public business URL <span class=\"subtle\">(if available)</span><input id=\"verification-url\" class=\"input\" type=\"url\" maxlength=\"500\" placeholder=\"https://business-site.example\"></label><p class=\"subtle\">Provide at least one reference or public HTTPS URL. Do not upload identity documents here; payout identity is handled separately by Stripe.</p><label class=\"label\">Anything else we should know? <span class=\"subtle\">(optional)</span><textarea id=\"verification-message\" class=\"textarea\" maxlength=\"500\"></textarea></label><div id=\"verification-status\"></div><button id=\"verification-submit\" class=\"primary wide\" type=\"submit\">Request verification</button></form></section>");
 }
 UI.app.innerHTML=html.join("");
