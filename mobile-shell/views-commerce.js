...
const orders=async()=>{
 ...
 try{
  ...
  if(UI.isSilentRefresh()){UI.toast("Could not refresh purchases. Showing the last loaded view.","warning");return;}
...
const order=async(payload)=>{
 ...
 UI.app.querySelectorAll("[data-accept]").forEach(button=>button.addEventListener("click",()=>{
  const itemId=button.dataset.accept;
  UI.modal("Accept & complete",
   "<div class=\"status warning\"><strong>Only continue after you have received and checked the part.</strong><div style=\"margin-top:5px\">This ends the 48-hour buyer-protection review window and makes the seller payout eligible immediately.</div></div>"+
   "<div class=\"button-row\" style=\"margin-top:12px\"><button id=\"accept-complete-confirm\" class=\"primary\" type=\"button\">Yes, accept & complete</button><button id=\"accept-complete-cancel\" class=\"secondary\" type=\"button\">Keep 48-hour protection</button></div>"
  );
  document.getElementById("accept-complete-confirm").addEventListener("click",()=>{UI.closeModal();void receiptAction(itemId,true,data.id);});
  document.getElementById("accept-complete-cancel").addEventListener("click",()=>UI.closeModal());
 }));
