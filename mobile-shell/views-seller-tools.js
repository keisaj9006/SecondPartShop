...
const sellerRequests=async()=>{
 ...
 const render=()=>{
  UI.app.innerHTML="<button class=\"back\" id=\"seller-requests-back\" type=\"button\">‹ Back to seller account</button><div class=\"section-head\"><div><p class=\"eyebrow\">Buyer demand</p><h2>Matched part requests</h2><p>"+items.length+" request"+(items.length===1?"":"s")+" loaded. SecondPart routes only relevant demand based on inventory, donor vehicles and fitment evidence.</p></div></div>"+(items.length?"<section>"+items.map(item=>"<article class=\"order-card\" style=\"margin-top:10px\"><div><div class=\"chips\">"+(item.categoryName?"<span class=\"chip\">"+C.escapeHtml(item.categoryName)+"</span>":"")+"<span class=\"pill warning\">"+(item.draftPartId?"Draft response":"Open request")+"</span><span class=\"chip green\">"+C.escapeHtml(matchLabel(item))+"</span></div><h3 style=\"margin:8px 0 0\">"+C.escapeHtml(item.queryText)+"</h3>"+(item.oemNumber?"<p class=\"subtle\">OE/OEM: <strong>"+C.escapeHtml(item.oemNumber)+"</strong></p>":"")+(vehicle(item)?"<p class=\"subtle\">"+C.escapeHtml(vehicle(item))+"</p>":"")+(item.notes?"<p class=\"subtle\" style=\"margin-top:8px\">"+C.escapeHtml(item.notes)+"</p>":"")+(Array.isArray(item.matchReasons)&&item.matchReasons.length?"<div class=\"status info\" style=\"margin-top:10px\"><strong>Why this matched</strong><br>"+C.escapeHtml(item.matchReasons.join(" · "))+"</div>":"")+"<p class=\"subtle\">Requested "+C.dateOnly(item.createdAt)+"</p><div class=\"button-row\" style=\"margin-top:10px\"><button class=\"primary small-button\" data-request-listing=\""+C.escapeHtml(item.id)+"\" type=\"button\">"+(item.draftPartId?"Resume draft":"Create matching listing")+"</button><button class=\"secondary small-button\" data-request-dismiss=\""+C.escapeHtml(item.id)+"\" type=\"button\">Not relevant · Dismiss</button></div></div></article>").join("")+"</section>"+(hasMore?"<button id=\"seller-requests-more\" class=\"secondary wide\" style=\"margin-top:14px\" type=\"button\">Load more buyer requests</button>":""):"<div class=\"empty\"><div class=\"empty-icon\">⌕</div><h3>No matched buyer requests</h3><p>New demand will appear here when SecondPart finds a relevant match for your inventory or donor vehicles.</p></div>");
  ...
  UI.app.querySelectorAll("[data-request-listing]").forEach(button=>button.addEventListener("click",()=>{const item=items.find(value=>value.id===button.dataset.requestListing);if(!item)return;if(item.draftPartId)UI.route("listingEditor",{id:item.draftPartId});else UI.route("listingEditor",{requestLead:item});}));
