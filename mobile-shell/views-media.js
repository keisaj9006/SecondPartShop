...
const listingEditor=async(payload={})=>{
 const requestLead=!payload.id&&payload.requestLead&&typeof payload.requestLead==="object"?payload.requestLead:null;
 ...
 const requestCategoryId=requestLead?.categoryId&&categories.some(value=>value.id===requestLead.categoryId&&value.isSelectable)?requestLead.categoryId:"";
 ...
 if(requestLead)html.push("<div class=\"status warning\" style=\"margin-bottom:12px\"><strong>Creating a listing for buyer demand</strong><div style=\"margin-top:4px\">"+C.escapeHtml(requestLead.queryText||"Requested part")+(requestLead.oemNumber?" · buyer supplied OE/OEM "+C.escapeHtml(requestLead.oemNumber):"")+"</div><div style=\"margin-top:4px;font-size:11px\">Verify all identifiers yourself before publishing.</div></div>");
 html.push("<form id=\"listing-editor-form\" class=\"form-grid\">"+
  "<section class=\"card\"><p class=\"eyebrow\">Part</p>"+
   "<label class=\"label\">Title<input id=\"le-title\" class=\"input\" maxlength=\"180\" value=\""+C.escapeHtml(item?.title||requestLead?.queryText||"")+"\" placeholder=\"e.g. Vauxhall Astra front left headlight\"/></label>"+
   "<label class=\"label\" style=\"display:block;margin-top:10px\">Category<select id=\"le-category\" class=\"select\"><option value=\"\">Choose specific category</option>"+categoryOptions(categories,item?.categoryId||requestCategoryId)+"</select></label>"+
 ...
 const formBody=()=>({
  sourceRequestId:!item&&requestLead?.id?requestLead.id:null,
