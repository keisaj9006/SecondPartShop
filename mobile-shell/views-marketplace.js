...
const home=async(payload={})=>{
 ...
 html.push("<section class=\"card flat\"><div class=\"row-between\"><div><p class=\"eyebrow\">Buy + Fit</p><h3 style=\"margin:4px 0\">Need the part fitted too?</h3><p class=\"subtle\">Find garages that accept customer-supplied recycled parts and request a separate labour quote.</p></div></div><div class=\"button-row\" style=\"margin-top:10px\"><button id=\"home-buy-fit\" class=\"secondary small-button\" type=\"button\">Find fitting garages</button>"+(C.state.session?"<button id=\"home-fitting-requests\" class=\"link-button\" type=\"button\">My fitting requests</button>":"")+"</div></section>");
...
const listing=async(payload)=>{
 ...
 let item,reputation=null,checkoutReady=false;
 try{
  const detailParams=new URLSearchParams();
  const vehicleQuery=activeVehicleQuery();
  if(vehicleQuery)new URLSearchParams(vehicleQuery).forEach((value,key)=>detailParams.set(key,value));
  const remembered=rememberedMarketplaceParams();
  if(remembered.pc)detailParams.set("pc",remembered.pc);
  const response=await C.api("/listings/"+encodeURIComponent(payload.slug)+(detailParams.toString()?"?"+detailParams.toString():""));
  item={
   ...response.item,
   compatibility:response.compatibility||response.item?.compatibility||null,
   distanceMiles:response.distance?.miles??response.item?.distanceMiles??null,
   distanceApproximate:Boolean(response.distance?.approximate??response.item?.distanceApproximate)
  };
 ...
 html.push("<section class=\"detail-hero\"><div class=\"detail-image\">"+(image?"<img src=\""+C.escapeHtml(image)+"\" alt=\""+C.escapeHtml(item.title)+"\"/>":"PART")+"</div><div class=\"detail-content\"><p class=\"eyebrow\">"+C.escapeHtml(UI.conditionLabel(item.condition))+" · "+C.escapeHtml(item.category&&item.category.name?item.category.name:"Part")+"</p><h1>"+C.escapeHtml(item.title)+"</h1><p class=\"listing-meta\">Sold by "+C.escapeHtml(item.seller.businessName)+(item.seller.verified?" · Verified seller":"")+(typeof item.distanceMiles==="number"?" · "+(item.distanceApproximate?"~":"")+C.escapeHtml(Number(item.distanceMiles).toFixed(1))+" mi away":"")+"</p>"+(reputation?"<button id=\"listing-member\" type=\"button\" class=\"secondary small-button\" style=\"margin-top:9px;text-align:left\">@"+C.escapeHtml(reputation.handle)+" · ★ "+C.escapeHtml(reputation.sellerRating===null||!reputation.sellerReviewCount?"New":Number(reputation.sellerRating).toFixed(1))+" ("+C.escapeHtml(reputation.sellerReviewCount)+") · "+C.escapeHtml(reputation.soldCount)+" sold · "+C.escapeHtml(reputation.boughtCount)+" bought</button>":"")+"<div class=\"detail-price\">"+C.money(item.pricePence)+"</div><div class=\"chips\"><span class=\"chip\">Stock "+C.escapeHtml(item.stock)+"</span><span class=\"chip\">"+(item.shippingPence?C.money(item.shippingPence)+" delivery":"Free delivery")+"</span>"+(item.collectionAvailable?"<span class=\"chip green\">Collection</span>":"")+(item.compatibility&&item.compatibility.label?"<span class=\"chip green\">"+C.escapeHtml(item.compatibility.label)+"</span>":"")+"</div><div class=\"spec-grid\"><div class=\"spec\"><small>OE/OEM</small><strong>"+C.escapeHtml(item.oemNumber||"Not listed")+"</strong></div><div class=\"spec\"><small>Part number</small><strong>"+C.escapeHtml(item.partNumber||"Not listed")+"</strong></div><div class=\"spec\"><small>Testing</small><strong>"+C.escapeHtml(C.human(item.testingStatus))+"</strong></div><div class=\"spec\"><small>Warranty</small><strong>"+C.escapeHtml(item.warrantyDays?item.warrantyDays+" days":"None stated")+"</strong></div></div><p style=\"font-size:12px;line-height:1.65;color:#4f5e57;margin-top:14px\">"+C.escapeHtml(item.description)+"</p></div></section>");
 html.push("<section class=\"card\"><div class=\"button-row\"><button id=\"listing-save\" class=\"secondary"+(saved?" selected":"")+"\" aria-pressed=\""+(saved?"true":"false")+"\" type=\"button\">"+(saved?"♥ Saved":"♡ Save part")+"</button>"+(!own?"<button id=\"listing-ask\" class=\"secondary\" type=\"button\">Ask seller</button><button id=\"listing-report\" class=\"link-button\" type=\"button\">Report listing</button>":"")+"</div>"+(!own?"<button id=\"listing-fit\" class=\"primary wide\" style=\"margin-top:12px\" type=\"button\">Buy + Fit · Find a garage</button><p class=\"subtle\" style=\"margin-top:7px\">Request a separate labour quote from a Buy + Fit garage for your selected vehicle.</p>"+(checkoutReady?"<form id=\"checkout-form\" class=\"form-grid\" style=\"margin-top:12px\"><div class=\"spec-grid\"><label class=\"label\">Quantity<input id=\"checkout-qty\" class=\"input\" type=\"number\" min=\"1\" max=\""+C.escapeHtml(Math.min(10,item.stock))+"\" value=\"1\"/></label><label class=\"label\">Delivery<select id=\"checkout-delivery\" class=\"select\"><option value=\"shipping\">Shipping</option>"+(item.collectionAvailable?"<option value=\"collection\">Collection</option>":"")+"</select></label></div><button class=\"lime-button wide\" type=\"submit\">Buy securely</button><p class=\"subtle\">Checkout opens Stripe securely. SecondPart never receives your card number.</p></form>":"<div class=\"status warning\" style=\"margin-top:12px\"><strong>Checkout temporarily unavailable</strong><div style=\"margin-top:4px\">This seller is finishing payment and payout setup. You can save the part or ask the seller a question in the meantime.</div></div>"):"<div class=\"status info\" style=\"margin-top:12px\">This is your own listing.</div>")+"</section>");
 UI.app.innerHTML=html.join("");
