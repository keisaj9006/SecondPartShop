...
const listing=async(payload)=>{
 ...
 html.push("<section class=\"card\"><div class=\"button-row\"><button id=\"listing-save\" class=\"secondary"+(saved?" selected":"")+"\" aria-pressed=\""+(saved?"true":"false")+"\" type=\"button\">"+(saved?"♥ Saved":"♡ Save part")+"</button>"+(!own?"<button id=\"listing-ask\" class=\"secondary\" type=\"button\">Ask seller</button><button id=\"listing-report\" class=\"link-button\" type=\"button\">Report listing</button>":"")+"</div>"+(!own?"<button id=\"listing-fit\" class=\"primary wide\" style=\"margin-top:12px\" type=\"button\">Buy + Fit · Find a garage</button><p class=\"subtle\" style=\"margin-top:7px\">Request a separate labour quote from a Buy + Fit garage for your selected vehicle.</p><form id=\"checkout-form\" class=\"form-grid\" style=\"margin-top:12px\"><div class=\"spec-grid\"><label class=\"label\">Quantity<input id=\"checkout-qty\" class=\"input\" type=\"number\" min=\"1\" max=\""+C.escapeHtml(Math.min(10,item.stock))+"\" value=\"1\"/></label><label class=\"label\">Delivery<select id=\"checkout-delivery\" class=\"select\"><option value=\"shipping\">Shipping</option>"+(item.collectionAvailable?"<option value=\"collection\">Collection</option>":"")+"</select></label></div><button class=\"lime-button wide\" type=\"submit\">Buy securely</button><p class=\"subtle\">Checkout opens Stripe securely. SecondPart never receives your card number.</p></form>":"<div class=\"status info\" style=\"margin-top:12px\">This is your own listing.</div>")+"</section>");
 UI.app.innerHTML=html.join("");
