...
| DOC-01 / P2 | Canonical/context documents contradict completed implementation and each other | pre-payments roadmap still says checkout missing/403; uppercase and lowercase product decisions collide on Windows | Preserve historical material with explicit status; reconcile current pointers; plan collision-safe rename | Low | Case-sensitive git handling | 0.5 day | No current instruction restarts onboarding/payments; source hierarchy clear |
