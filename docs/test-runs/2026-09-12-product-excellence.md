...
Batch6 Preview on00d2fd7, CI34710804269 success, READY dpl_9b11i5fpcm3gDtjrfDaV95iUpkUz (second-part-shop-6bjijylmb-joannakwapis11-5369.vercel.app): anonymous768x1024 now clean menu/wordmark,1280x800 desktop remains clean,390x844 and390x480 clean. Synthetic root-font200% at390x480 retains square badge and accessible truncated wordmark; client/scroll both390. This is not native zoom/device RC.

QA Buyer actual389x844/client378 fixes prior overlap: brand right165.33px, Search left200.06px. Both Garage/Seller destinations remain in menu; Escape from inside returns focus to Open navigation and expanded=false. Authenticated1279x799 menu layout clean. At actual1281x800/client1269, desktop still truncates the wordmark and wraps several primary links into2lines because account identity/actions occupy too much width. This remaining confirmed responsive issue must be fixed and rechecked before full Batch6 PASS. No account/listing mutation. Seller form remains gated on correct QA Seller login.
Additional read-only responsive evidence: QA Garage at actual389x844 shows usable Add a vehicle and saved vehicle card/Use this vehicle with truthful representative-image disclaimer; no Garage record changed. Anonymous product detail at390x480 client=scroll390 has reachable page structure and stable bottom navigation. /saved corroborates existing DQ200 pressed=true while /account recent card remains false; final integration repair required. Both Task6 aliases confirmed on dpl_9b11i5fpcm3gDtjrfDaV95iUpkUz. Temporary CUA viewport override reset.

## Batch7 local metadata implementation

Added bounded public Home/Part/Seller metadata, Preview/nonproduction noindex and generic noindex for current private route families. Home canonical/Open Graph live only on the Home page, preventing inheritance into unrelated/private routes. No origin configured; canonical/URL-dependent schema remains omitted without approved production environment and strict origin validation. Active public listings emit exact GBP price, condition/stock and safe Product/Offer JSON-LD; no invented rating/fitment claims. React request cache shares public lookup between metadata and page.

Focused metadata14/14 and affected legacy route+metadata63/63 pass. Initial full291 had18 failures from old VM harnesses rejecting new imports; three dependency maps were updated without weakening their existing navigation/compatibility assertions. Root build and all14validators pass; lint0errors/3legacywarnings and typecheck0. Final full-suite rerun and independent review are the acceptance gates before commit; no Preview SEO claim yet.
Batch7 final full291/291 PASS; post-harness lint0errors/3legacywarnings and typecheck0; build0/all14validators0. Independent scoped review PASS and independentlyreran63affectedtests PASS. No application-code finding in that review. Exact Preview metadata acceptance pending commit/deployment.
...
| 5 Seller/CSV | PASS: engineering plus QA Seller CSV Preview, file retention and validation retries | Actual import/post-write recovery and transient provider error cause remain unproven |
| 6 Responsive quality | PASS: confirmed header/checkbox defects fixed; representative Buyer and QA Seller form viewports verified | Native text zoom and exhaustive page/device combinations remain RC |
...
Next best step: the two QA Seller browser gates are complete. A separately authorized consolidated RC/provider/device round and production-origin/legal/support/professional-seller inventory decisions remain; no nonblocked P0/P1 implementation remains in this plan. Engineering is close to a restricted web beta; a credible calendar estimate for closed beta cannot be inferred from green code checks while these external gates are unsigned. This phase does not claim SecondPart1.0 or Android launch readiness.

## QA Seller follow-up — 12 September 2026

Resumed at clean91c84ba on rebuild-nextjs, origin0/0. The supplied Preview session was verified as the distinct QA Seller: account owner cf2ef681-9ced-4fb8-9a50-efd5668138b0, seller dashboard SecondPart QA Seller. Buyer/Moira and the sold Stripe fixture were not used for seller actions.

Actual CSV Preview on the existing application:

- Selected synthetic qa-csv-preview-valid.csv through the normal file chooser, then Preview CSV. Result:1 row,1 valid,0 rejected, stable reference QA-PREVIEW-20260912, category alternators and price3.25.
- One subsequent no-reselection preview returned Existing seller references could not be checked. Source localizes that message to the returned error from get_existing_csv_inventory_references, before any import batch/part write. Underlying provider error detail is unavailable from this UI response; no network/credential/root-cause claim is made and no outage was induced.
- Missing-reference file:0 valid,1 rejected, row2 explicitly says seller_reference is required. Repeating Preview without selecting a file again preserved the rejection result.
- Reselecting the valid file succeeded; repeating it without reselection also succeeded with1 valid/0 rejected. This establishes actual browser file retention across successful previews and validation errors. The intermittent lookup error was fail-closed, not masked by a write or automatic retry.
- Never clicked Import valid rows as drafts. Recent imports remained empty; dashboard still showed the same2 listings, unsold QA draft stock1 and sold fixture stock0. No CSV import/report/batch was created through this workflow. Post-write clear/finalization recovery remains synthetic evidence, not real-provider E2E.

Added one synthetic regression for the already-existing lookup-error boundary: both preview/import modes return bounded error and fileReset=retain with zero part/batch writes; a later preview using the same File succeeds without writes. Provider payload is not exposed.11/11 importer tests pass; this is added coverage of existing behavior, not a claimed RED/GREEN production fix or proof of the transient provider cause.

Seller-form responsive observation used only existing unpublished fixture760fdafa-e589-449e-9296-397f76c74bd2. No save, photo removal, upload, AI request, fitment addition or publication. Actual viewport/client widths:320x844/309;389x844/378;389x480/378;769x1024/757;1025x800/1014;1281x800/1269. Document width matched client width, and form input/select/textarea/button/fieldset bounds stayed inside it. At389x480 Save listing ended at382.94px, above the bottom navigation. Tablet/desktop category, photo and final controls remained readable. Native selected-option text may truncate within selects while the selected-category summary remains visible.

One confirmed additional defect: expanded fitment confirmation checkbox flex-shrank to4.73px wide at389x844, versus12.996px height. Source seller-compatibility-editor.tsx uses a shrinking input beside long text. A minimal nonshrinking size correction and regression are in progress; responsive gate is not final PASS until corrected Preview is checked. No fitment policy relaxation is authorized.

Browser Ctrl+plus attempts did not change observed viewport/root font (16px). No enlarged-text verification is claimed from that unsupported shortcut; native zoom/text-size remains the previously deferred RC check. Temporary override will be reset after corrected Preview acceptance.

Follow-up local verification: root25/25 focused tests (11 importer,12 listing retry,2 seller confirmation) and all14 static validators pass. The checkbox correction changes only sizing/shrink/accent classes; native confirmation semantics remain. A scoped independent review and fresh build are running. No additional nonblocked implementation item remains in the approved Product Excellence task list after this measured defect; canonical/domain publication and the explicitly deferred physical/provider/zoom/release gates must not be silently converted into new authorized work.

Measured checkbox repair verification: RED on the missing size/nonshrinking class, GREEN2/2 component tests, root25/25 focused tests, lint0errors/3 pre-existing warnings, typecheck0, build0, all14 static validators0, staged diff-check0. Independent scoped review approved all three implementation/test files with no actionable findings. Commits6402f47 and a54f205 separate the UI correction from CSV failure coverage. Exact corrected Preview acceptance follows; no state-changing seller action has been performed.

### QA Seller follow-up acceptance: PASS for CSV Preview and responsive form

Corrected code a54f20545aaa7a63e51d2613cffd06ae9365ed4a passed CI34719107428 and deployed READY as dpl_8pzGER2fpU5zeDi37wyyJU7qeC69 (second-part-shop-g1ydj5sgu-joannakwapis11-5369.vercel.app). Both aliases were confirmed on that exact deployment after stable assignment. Root build/typecheck/lint and25 focused tests/14 validators passed; independent review found no actionable issue.

Actual post-deployment checkbox measurements:20.00x20.00px, flexShrink0 at389x844. The same20x20 size and no out-of-bounds form controls or document overflow were confirmed at320x844,389x480,769x1024,1025x800 and1281x800. Visible screenshots confirm the warning text wraps beside a legible checkbox. Native Space toggled false→true→false; Add confirmed vehicle stayed disabled because no exact vehicle was selected. No fitment was added or listing saved. The existing draft remained price3.25/stock1/warranty90/Draft with its3 photos. Viewport override was reset; QA Seller session remains available.

This supersedes the earlier QA Seller login/CSV/form pending notes. Batch5's safe Preview/file-retention gate and Batch6's seller viewport gate are now closed. Actual post-write import, provider failure cause/recovery, native enlarged text/screen reader/device and other explicitly deferred RC gates remain unproven. No further nonblocked P0/P1 implementation remains in this approved Product Excellence plan. Next work requires the separately authorized RC round or domain/business/release decisions; no new architecture, Production configuration or payment work was started.
Final code CI34719107428 also ran the complete suite:304/304 passed,0 failed. This is CI evidence; local reruns were intentionally limited to25 relevant tests plus required lint/typecheck/build/static validators. A final documentation-only commit will record this evidence and must have its own exact-HEAD CI/Preview alias confirmation.
