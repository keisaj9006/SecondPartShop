...
- Correction from the follow-up audit: `docs/PRODUCT_DECISIONS.md` and `docs/product-decisions.md` differ by underscore versus hyphen as well as case, so they do not collide on case-insensitive checkouts. No rename is needed. Their different content still needs clear current/historical status.
