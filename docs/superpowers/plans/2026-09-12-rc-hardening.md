...
| 3 Last-stock concurrency | VERIFIED scoped engineering evidence: canonical checkout SQL was exercised by two genuinely overlapping PostgreSQL sessions against one stock unit; exactly one order/item/reservation event won and the losing buyer created no order. CI `34868502058` passed the dedicated `last-stock-concurrency` job at `23929224861d91a82a8be96132838425f78c88d6`. Full Preview/provider/UI Scenario G remains P0 unsigned. | Preserve the dedicated two-connection PostgreSQL job. Complete provider/UI Scenario G separately before release sign-off; do not relabel isolated database proof as full commerce E2E. |
...
### First repair batch — local verification complete, Preview pending

- Canonical register committed as `c68e25f`; privacy code/SQL regression as `08aafe2`; isolated scale probe/evidence as `a5ba0c4`. These commits were local at this checkpoint, not yet a deployment claim.
- Privacy and scale independent specification/quality reviews PASS. Review identified insufficient blocked/mismatched donor/import assertions; these were strengthened and rereviewed PASS. Privacy's actual destructive E2E remains unsigned.
- **Privacy deployment BLOCKED by automatic approval review.** Two attempts to install the reviewed function definition were rejected because future execution deletes/scrubs retained data. Read-only checks confirmed zero deletion requests and zero top-level data mutation/invocation in the migration; the existing definition remains unchanged. Explicit approval for this exact migration/project was requested. Do not retry through another API or execute deletion to bypass the rejection.
- Checkout independent review initially reproduced two further defects despite 55 passing tests: resuming a locally cancelled order exposed its still-payable URL; malformed successful create responses without a session ID were mistakenly compensated as null-session cancellation. Both were corrected, with truthful order-state banners. Independent rereview: **103/103 focused tests PASS**, specification/quality approved locally.
- Checkout session-CAS definition successfully installed in QA Supabase as remote migration `20260912222402 / checkout_cancellation_session_guard` (source `20260912215018_checkout_cancellation_session_guard.sql`). Read-only inspection confirms row lock, null-safe session comparison, anon/authenticated denial and service-role execution. No cancellation, payment or stock mutation was invoked by this deployment.
- Receipt review reproduced one extra input-boundary defect: explicit SQL `p_accept_now=NULL` bypassed the retry no-op and duplicated the receipt event. The flag is now normalized once; exact regression and independent rereview PASS. Receipt repair also preserves an in-flight payout claim and prevents detached/null-buyer access.
- Final integrated tests after the receipt NULL correction: **415/415 PASS**, zero skipped/failed; lint exit 0 with three unchanged legacy warnings; typecheck and final production build exit 0. All 14 validators exit 0 with retained per-validator logs. An earlier validator run passed all commands but its PowerShell log redirection failed; the retained rerun resolves that evidence issue.
- Checkout code committed as `6b8b525`; receipt repair as `7acc4dc`. Receipt definition installed as remote migration `20260912223112 / buyer_receipt_idempotency` (source `20260912220057_buyer_receipt_idempotency.sql`). Deployed read-only checks: anon false, authenticated true, service role false; NULL normalization and receipt no-op present; `auto_release_hours=48`. No receipt, payout or payment operation was invoked.
- CI, exact Preview deployment and runtime checks for this batch remain pending the push. A provider cancellation/decline/refund/dispute/concurrency/elapsed-window PASS is not implied by these repairs or definition checks.

### Isolated last-stock concurrency evidence — 2026-09-14

- At `23929224861d91a82a8be96132838425f78c88d6`, GitHub Actions run `34868502058` completed successfully with a dedicated PostgreSQL 17 `last-stock-concurrency` job.
- `scripts/verify-last-stock-concurrency-postgres.mjs` loads the canonical `prepare_checkout_order` definition, asserts the `FOR UPDATE` row lock, starts two independent `psql` processes and deliberately holds buyer A's transaction open while buyer B attempts the same stock-1 listing.
- Buyer A returns one real order ID. Buyer B fails closed only after the first transaction commits. Final state is exactly `stock=0`, `status=reserved`, `orders=1`, `items=1`, `reservation_events=1`, `buyer_a_orders=1`, `buyer_b_orders=0`.
- This closes the previously unsigned **isolated database concurrency proof**. It does **not** close the full Preview/provider/UI Scenario G, cancellation/expiry provider evidence or broader hosted-load gate.

The isolated scale report is `docs/test-runs/2026-09-12-search-scale.md`: six correctness cases across 25,000 matches, including compact OEM plus category, bounded sentinel pages and global deep-page ordering. Measured single-query PGlite times range from 468.0 to 3,030.9 ms in the final run; no hosted performance improvement or SLO is claimed.
