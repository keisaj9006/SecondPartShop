...
- [ ] Apply the smallest shared style or local layout fix per confirmed defect; prefer readable text, reachable CTA and visible focus. Do not add decorative assets.
- [ ] Add regression coverage for changed behavior/layout contract; repeat affected viewports, tests/lint/typecheck/review. Real physical device/screen reader execution remains RC.

## Task 7: Safe metadata and SEO preparation

- [ ] Inspect public route data and production-origin helper. Document canonical/landing decisions requiring actual domain/business input as blocked.
- [ ] Add route-appropriate metadata without duplicate expensive fetches. Set Preview/nonproduction noindex; use no fake canonical. Private account/seller workspace metadata must not leak personal data.
- [ ] If data supports it, render safe public Product/Offer/Breadcrumb JSON-LD with exact price/currency/status and existing images; omit unsupported brand/ratings/availability claims. Escape `<` in serialized JSON. No autogenerated empty landing pages.
- [ ] Test synthetic metadata/structured data and deployment robots/meta behavior. Full tests/build/14 validators, final independent integration review, CI/Preview/alias verification and final evidence report.
