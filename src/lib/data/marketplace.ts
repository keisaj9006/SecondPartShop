...
const sellerFrom=(s:RawSeller):Seller=>({id:s.id,ownerId:s.owner_id,businessName:s.business_name,slug:s.slug,location:s.location,postcode:s.postcode,description:s.description,verified:Boolean(s.verified_at),sellerType:s.seller_type,businessKind:s.business_kind});
