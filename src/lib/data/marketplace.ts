...
import type { Category,Fitment,Listing,ListingImage,MarketplaceFilters,Seller,Vehicle,VehicleDataStatus } from "@/lib/types";

export type DataResult<T>={data:T;error:string|null;configured:boolean};
type RawCategory={id:string;name:string;slug:string;is_transmission_related:boolean};
...
export async function getCategories():Promise<Category[]>{if(!isSupabaseConfigured())return [];const supabase=await createSupabaseServerClient();const {data}=await supabase.from("categories").select("id,name,slug,is_transmission_related").order("name");return (data??[]).map(c=>categoryFrom(c as RawCategory));}
...
export async function getFilterOptions(){if(!isSupabaseConfigured())return {families:[] as string[],codes:[] as string[]};const supabase=await createSupabaseServerClient();const {data}=await supabase.from("parts").select("gearbox_family,gearbox_code").eq("status","active");const families=(data??[]).map(p=>p.gearbox_family).filter((v):v is string=>Boolean(v));const codes=(data??[]).map(p=>p.gearbox_code).filter((v):v is string=>Boolean(v));return {families:[...new Set(families)].sort(),codes:[...new Set(codes)].sort()};}
