...
import type { Category,Fitment,Listing,ListingImage,MarketplaceFilters,SearchSuggestionGroups,Seller,SellerBusinessKind,Vehicle,VehicleDataStatus } from "@/lib/types";
...
type RawSeller={id:string;owner_id:string|null;business_name:string;slug:string;location:string;postcode?:string|null;description:string;verified_at:string|null;seller_type:"business"|"private";business_kind:SellerBusinessKind|null};
...
const sellerFrom=(s:RawSeller):Seller=>({id:s.id,ownerId:s.owner_id,businessName:s.business_name,slug:s.slug,location:s.location,postcode:s.postcode??null,description:s.description,verified:Boolean(s.verified_at),sellerType:s.seller_type,businessKind:s.business_kind});
...
function listingFrom(row:RawListing):Listing{const category=one(row.categories);const seller=one(row.sellers);const images:ListingImage[]=(row.part_images??[]).sort((a,b)=>a.position-b.position).map(i=>({id:i.id,url:publicImageUrl(i.storage_path),alt:i.alt_text,position:i.position}));const fitments:Fitment[]=(row.part_fitments??[]).flatMap(f=>{const v=f.vehicles?one(f.vehicles):null;return v?[{vehicle:vehicleFrom(v),notes:f.notes}]:[];});return {id:row.id,sellerId:row.seller_id,categoryId:row.category_id,donorVehicleId:row.donor_vehicle_id,sourceChannel:row.source_channel,sourceExternalId:row.source_external_id,importBatchId:row.import_batch_id,slug:row.slug,title:row.title,description:row.description,manufacturer:row.manufacturer,partNumber:row.part_number,oemNumber:row.oem_number,gearboxFamily:row.gearbox_family,gearboxCode:row.gearbox_code,condition:row.condition,pricePence:row.price_pence,shippingPence:row.shipping_pence,stock:row.stock,status:row.status,dispatchDays:row.dispatch_days,testingStatus:row.testing_status,warrantyDays:row.warranty_days,conditionNotes:row.condition_notes,damageNotes:row.damage_notes,collectionAvailable:row.collection_available,deliveryDaysMin:row.delivery_days_min,deliveryDaysMax:row.delivery_days_max,category:categoryFrom(category),seller:sellerFrom(seller),images,fitments};}
...
async function cardListingsFromRows(supabase:ServerSupabase,rows:unknown[]):Promise<Listing[]>{
 if(!rows.length)return [];
 const rawRows=rows as unknown as Array<Omit<RawListing,"part_images"|"part_fitments">>;
...
export async function getSellers():Promise<Seller[]>{if(!isSupabaseConfigured())return [];const supabase=await createSupabaseServerClient();const {data}=await supabase.from("sellers").select(publicSellerSelect).order("business_name");return (data??[]).map(s=>sellerFrom(s as RawSeller));}
export async function getSellerBySlug(slug:string):Promise<Seller|null>{if(!isSupabaseConfigured())return null;const supabase=await createSupabaseServerClient();const {data}=await supabase.from("sellers").select(publicSellerSelect).eq("slug",slug).maybeSingle();return data?sellerFrom(data as RawSeller):null;}
