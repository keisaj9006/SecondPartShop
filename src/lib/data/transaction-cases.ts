...
export async function getTransactionCases():Promise<TransactionCase[]>{
 ...
 const {data,error}=await supabase
  ...
  .select("id,order_item_id,case_type,reason,details,status,previous_fulfilment_status,seller_response,resolution,resolution_notes,created_at,resolved_at,order_items(parts(title,slug),sellers(business_name,slug),orders(buyer_id))")
