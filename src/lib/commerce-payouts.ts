...
import { createSellerTransfer,isStripeCheckoutConfigured } from "@/lib/stripe-payments";
...
export async function releaseDuePayoutItem(orderItemId:string){
 ...
 const {data:item,error:itemError}=await admin
  .from("order_items")
  .select("id,order_id,seller_id,seller_net_pence,payout_status,release_eligible_at,funds_released_at")
  .eq("id",orderItemId)
  .maybeSingle();
 ...
 if(item.payout_status!=="scheduled"||!item.release_eligible_at||new Date(item.release_eligible_at).getTime()>Date.now())return {released:false,reason:"not_due"} as const;
 ...
 const {error:markError}=await admin.rpc("mark_order_item_payout_released",{
...
export async function releaseDuePayouts(limit=100){
 ...
 return {checked:data?.length??0,released,deferred};
}
