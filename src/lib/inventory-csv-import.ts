...
async function validateCsv(file:File,sellerId:string,supabase:SupabaseClient<Database>){
 ...
 for(let index=0;index<parsed.rows.length;index+=1){
  ...
  valid.push({
   row:rowNumber,title,description,categoryId:category!.id,
   condition:condition as ValidatedRow["condition"],
   pricePence:pricePence!,shippingPence:shippingPence!,stock:stock!,
   manufacturer:nullable(row.manufacturer,160),partNumber:nullable(row.part_number,160),oemNumber:nullable(row.oem_number,160),
   ...
   dispatchDays:dispatchDays!,testingStatus:testingStatus as ValidatedRow["testingStatus"],warrantyDays:warrantyDays!,
   conditionNotes:nullable(row.condition_notes,500),damageNotes:nullable(row.damage_notes,500),
   collectionAvailable:collectionAvailable!,deliveryDaysMin,deliveryDaysMax,
   donorVehicleId:normalizedDonor?donorByReg.get(normalizedDonor)??null:null,sellerReference
  });
 }
...
export async function processSellerInventoryCsv({
 file,sellerId,supabase,mode
}:{
 file:File;
 sellerId:string;
 ...
 const {data:batch,error:batchError}=await supabase.from("seller_inventory_imports").insert({
  seller_id:sellerId,source_channel:"csv",filename:file.name.slice(0,255),status:"failed",
 ...
 const payload=validation.rows.map(row=>({
  seller_id:sellerId,category_id:row.categoryId,donor_vehicle_id:row.donorVehicleId,
  slug:slugify(row.title)+"-"+crypto.randomUUID().slice(0,8),title:row.title,description:row.description,
  manufacturer:row.manufacturer,part_number:row.partNumber,oem_number:row.oemNumber,
  gearbox_family:row.gearboxFamily,gearbox_code:row.gearboxCode,condition:row.condition,
  price_pence:row.pricePence,shipping_pence:row.shippingPence,stock:row.stock,status:"draft" as const,
  dispatch_days:row.dispatchDays,testing_status:row.testingStatus,warranty_days:row.warrantyDays,
  condition_notes:row.conditionNotes,damage_notes:row.damageNotes,collection_available:row.collectionAvailable,
  delivery_days_min:row.deliveryDaysMin,delivery_days_max:row.deliveryDaysMax,source_channel:"csv",
  source_external_id:row.sellerReference,import_batch_id:batch.id
 }));
