...
export function MarketplaceHome({listings,categories,vehicles,garageVehicles,recentlyViewed,signedIn,filters,selectedCatalogue,savedIds,error,configured,pagination,currentPage,freshVehicleSelection=false}:{listings:Listing[];categories:Category[];vehicles:Vehicle[];garageVehicles:GarageVehicle[];recentlyViewed:Listing[];signedIn:boolean;filters:MarketplaceFilters;selectedCatalogue:VehicleCatalogueSelection|null;savedIds:string[];error:string|null;configured:boolean;pagination:{offset:number;limit:number;returned:number;total:number|null;hasMore:boolean};currentPage:number;freshVehicleSelection?:boolean}){
