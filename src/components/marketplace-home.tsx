...
export function MarketplaceHome({listings,categories,vehicles,catalogueModels,garageVehicles,recentlyViewed,signedIn,filters,selectedCatalogue,savedIds,error,configured,pagination,currentPage}:{listings:Listing[];categories:Category[];vehicles:Vehicle[];catalogueModels:VehicleCatalogueModelOption[];garageVehicles:GarageVehicle[];recentlyViewed:Listing[];signedIn:boolean;filters:MarketplaceFilters;selectedCatalogue:VehicleCatalogueSelection|null;savedIds:string[];error:string|null;configured:boolean;pagination:{offset:number;limit:number;returned:number;total:number|null;hasMore:boolean};currentPage:number}){
