...
export function MarketplaceHome({listings,categories,vehicles,catalogueModels,families,codes,filters,selectedCatalogue,savedIds,error,configured}:{listings:Listing[];categories:Category[];vehicles:Vehicle[];catalogueModels:VehicleCatalogueModelOption[];families:string[];codes:string[];filters:MarketplaceFilters;selectedCatalogue:VehicleCatalogueSelection|null;savedIds:string[];error:string|null;configured:boolean}){
 ...
 return <main>
  <section className="dot-grid overflow-hidden bg-[#173c31] text-white">
   <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 lg:grid-cols-[.9fr_1.1fr] lg:py-20">
    <div className="animate-in">
     ...
     <p className="mt-6 max-w-xl text-lg leading-7 text-white/72">Find verified automotive parts by your vehicle, OEM number or part number — supplied by trusted UK garages.</p>
     <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-sm text-white/75"><span className="flex items-center gap-2"><BadgeCheck size={18} className="text-[#d4f44d]"/>Verified garages</span><span className="flex items-center gap-2"><ShieldCheck size={18} className="text-[#d4f44d]"/>Vehicle compatibility</span><span className="flex items-center gap-2"><Truck size={18} className="text-[#d4f44d]"/>UK-wide sellers</span></div>
    </div>
  ...
  <section id="marketplace" className="mx-auto max-w-7xl px-4 py-14 sm:px-6">
   <div className="mb-7"><p className="mb-2 text-xs font-black uppercase tracking-[.2em] text-[#287154]">Marketplace</p><h2 className="text-3xl font-black tracking-[-.04em] sm:text-4xl">Available parts</h2><p className="mt-2 text-[#63706a]">{listings.length} listing{listings.length===1?"":"s"} match the current search</p></div>
   <MarketplaceFiltersPanel filters={filters} categories={categories} families={families} codes={codes}/>
   ...
   {listings.length?<div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{listings.map(item=><ProductCard key={item.id} item={item} saved={savedIds.includes(item.id)}/>)}</div>:!error&&<div className="mt-8 rounded-3xl border border-dashed border-black/20 bg-white py-20 text-center"><Search className="mx-auto mb-4 text-[#63706a]"/><h3 className="text-xl font-bold">No matching parts</h3><p className="mx-auto mt-2 max-w-xl text-[#63706a]">{selectedCatalogue?"This vehicle is available in the UK catalogue, but no seller has linked a compatible part to this exact catalogue variant yet.":"Clear one or more filters and try again."}</p></div>}
  </section>
 </main>;
}
