...
export function ResendVerificationForm({returnTo}:{returnTo?:string}){
 ...
 return <div className="w-full max-w-md rounded-3xl border border-black/10 bg-white p-8 shadow-xl">
  ...
  <Link href={safeReturnTo?`/account?returnTo=${encodeURIComponent(safeReturnTo)}`:"/account"} className="mt-5 inline-block text-sm font-black underline">Back to sign in</Link>
 </div>;
}
