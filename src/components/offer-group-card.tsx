...
export function OfferGroupCard({group,contextQuery=""}:{group:OfferGroup;contextQuery?:string}){
 ...
 const prices=group.listings.map(item=>item.pricePence);
 ...
 const bestCompatibility=group.listings.find(item=>item.compatibility?.level==="confirmed")?.compatibility??group.listings.find(item=>item.compatibility?.level==="buyer_verified")?.compatibility??group.listings.find(item=>item.compatibility?.level==="family_match")?.compatibility??null;
