 ...
 return <header ref={headerRef} className="app-topbar sticky top-0 z-50 border-b border-black/10 bg-[#fbfcfa]/95 backdrop-blur-xl">
  <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between px-3 py-2.5 sm:min-h-18 sm:px-6 sm:py-3">
   ...
   <div className="flex shrink-0 items-center gap-0.5 sm:gap-1">
    ...
    <button ref={mobileTriggerRef} type="button" aria-label={mobileOpen?"Close navigation":"Open navigation"} aria-expanded={mobileOpen} aria-controls="header-mobile-navigation" onClick={()=>{setMobileOpen(value=>!value);setCategoriesOpen(false);setMobileCategories(false);}} className="rounded-full p-2.5 md:hidden">{mobileOpen?<X size={21}/>:<Menu size={21}/>}</button>
   </div>
  </div>

  {categoriesOpen&&<div ref={desktopCategoriesPanelRef} id="header-desktop-categories" className="absolute left-0 right-0 top-full hidden border-b border-black/10 bg-[#f8f7f2] shadow-2xl md:block">
   <div className="mx-auto max-w-7xl px-6 py-6"><CategoryBrowser categories={categories} onSelect={selectCategory}/></div>
  </div>}

  {mobileOpen&&<div ref={mobilePanelRef} id="header-mobile-navigation" className="border-t border-black/10 bg-white px-4 py-4 shadow-xl md:hidden">
