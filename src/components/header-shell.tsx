 ...
 return <header ref={headerRef} className="sticky top-0 z-50 border-b border-black/10 bg-[#fbfcfa]/95 backdrop-blur-xl">
  ...
  {mobileOpen&&<div className="border-t border-black/10 bg-white px-4 py-4 shadow-xl md:hidden">
   {!mobileCategories?<nav className="grid gap-1 text-sm font-bold">
    ...
    <Link className="rounded-xl px-3 py-3 hover:bg-black/5" href="/saved" onClick={()=>setMobileOpen(false)}>Saved parts</Link>{user&&<><Link className="rounded-xl px-3 py-3 hover:bg-black/5" href="/garage" onClick={()=>setMobileOpen(false)}>SecondPart Garage</Link><Link className="rounded-xl px-3 py-3 hover:bg-black/5" href="/requests" onClick={()=>setMobileOpen(false)}>Part requests</Link></>}
