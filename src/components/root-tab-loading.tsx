...
export function RootTabLoading({variant}:{variant:RootTabLoadingVariant}){
 const titles={
  garage:["Your garage","Saved vehicles and compatibility"],
  purchases:["Purchases","Orders, delivery and buyer protection"],
  inbox:["Messages","Part questions and conversations"],
  account:["Your account","Buyer and seller tools"]
 } as const;
 const [title,subtitle]=titles[variant];

 return <main aria-busy="true" className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
  <div className="animate-pulse">
   <div className="h-3 w-24 rounded-full bg-[#287154]/20"/>
   <div className="mt-3 h-9 w-56 max-w-[75vw] rounded-xl bg-black/10" aria-label={title}/>
   <div className="mt-3 h-4 w-72 max-w-[85vw] rounded-lg bg-black/5" aria-label={subtitle}/>
