...
export function FoundingSellerApplicationForm({source="website"}:{source?:string}){
 ...
 return <form action={action} className="rounded-[28px] border border-black/10 bg-white p-5 sm:p-7">
  ...
  <div className="grid gap-4 sm:grid-cols-2">
   ...
   <label className="text-sm font-black">Business email<input required type="email" maxLength={320} name="email" autoComplete="email" className="mt-2 w-full rounded-xl border border-black/15 px-4 py-3"/></label>
   <label className="text-sm font-black">Phone <span className="font-normal text-[#63706a]">(optional)</span><input maxLength={80} name="phone" autoComplete="tel" className="mt-2 w-full rounded-xl border border-black/15 px-4 py-3"/></label>
   <label className="text-sm font-black">Business name<input required minLength={2} maxLength={160} name="businessName" autoComplete="organization" className="mt-2 w-full rounded-xl border border-black/15 px-4 py-3"/></label>
   <label className="text-sm font-black">Business type<select required defaultValue="" name="businessKind" className="mt-2 w-full rounded-xl border border-black/15 bg-white px-4 py-3"><option value="" disabled>Choose one</option><option value="breaker">Vehicle breaker</option><option value="atf">ATF / vehicle dismantler</option><option value="garage">Garage / workshop</option><option value="parts_business">Parts business / retailer</option><option value="other">Other automotive business</option></select></label>
   <label className="text-sm font-black">Postcode<input required maxLength={20} name="postcode" autoComplete="postal-code" className="mt-2 w-full rounded-xl border border-black/15 px-4 py-3 uppercase"/></label>
   <label className="text-sm font-black sm:col-span-2">Website or eBay shop URL <span className="font-normal text-[#63706a]">(optional)</span><input maxLength={500} name="website" inputMode="url" className="mt-2 w-full rounded-xl border border-black/15 px-4 py-3" placeholder="https://..."/></label>
   <label className="text-sm font-black">Approx. active parts in stock <span className="font-normal text-[#63706a]">(optional)</span><input type="number" min={0} max={10000000} step={1} name="estimatedActiveParts" className="mt-2 w-full rounded-xl border border-black/15 px-4 py-3" placeholder="e.g. 5000"/></label>
   <label className="text-sm font-black">Preferred inventory onboarding<select required defaultValue="unsure" name="importInterest" className="mt-2 w-full rounded-xl border border-black/15 bg-white px-4 py-3"><option value="unsure">Not sure yet</option><option value="ai_manual">AI / manual listing</option><option value="csv">CSV bulk import</option><option value="ebay">Existing eBay inventory</option><option value="api">API / system integration</option></select></label>
  </div>
  ...
  <fieldset className="mt-5">
   ...
   <div className="mt-3 grid gap-2 sm:grid-cols-3">
    {[["ebay","eBay"],["own_website","Own website"],["physical_counter","Trade counter / shop"],["facebook","Facebook / social"],["other","Other"],["none","Not online yet"]].map(([value,label])=><label key={value} className="flex items-center gap-2 rounded-xl bg-[#f8f7f2] p-3 text-sm font-bold"><input type="checkbox" name="channels" value={value}/>{label}</label>)}
   </div>
  </fieldset>
