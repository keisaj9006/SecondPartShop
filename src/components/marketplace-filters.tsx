...
export function MarketplaceFiltersPanel({filters,categories,families,codes}:{filters:MarketplaceFilters;categories:Category[];families:string[];codes:string[]}){
 ...
 return <form method="get" action="/" className="grid gap-3 rounded-2xl border border-black/10 bg-white p-4 sm:grid-cols-2 lg:grid-cols-6">
  ...
  {filters.vehicle&&<input type="hidden" name="vehicle" value={filters.vehicle}/>}{filters.catalogueVariant&&<input type="hidden" name="cv" value={filters.catalogueVariant}/>}{filters.catalogueYear!==undefined&&<input type="hidden" name="cy" value={filters.catalogueYear}/>}{filters.catalogueFuel&&<input type="hidden" name="cf" value={filters.catalogueFuel}/>}{filters.catalogueEngineSize!==undefined&&<input type="hidden" name="ce" value={filters.catalogueEngineSize}/>}<button className="rounded-xl bg-[#173c31] px-5 py-2.5 text-sm font-black text-white">Apply filters</button><Link href="/#marketplace" className="grid place-items-center rounded-xl border border-black/12 px-5 py-2.5 text-sm font-bold">Clear</Link>
 </form>;
}
