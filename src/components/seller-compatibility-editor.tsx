...
export function SellerCompatibilityEditor({initialFitments=[]}:{initialFitments?:CatalogueFitmentSelection[]}){
 ...
 return <details open={fitments.length>0} className="rounded-2xl border border-black/10 bg-[#f8f7f2] p-4 lg:col-span-2">
  ...
  <div className="mt-4 rounded-2xl border border-black/10 bg-white p-4">
   ...
   <label className="mt-4 flex items-start gap-3 rounded-xl bg-[#f8f7f2] p-3 text-sm"><input type="checkbox" checked={confirmed} onChange={event=>setConfirmed(event.target.checked)} className="mt-1 h-5 w-5 shrink-0 accent-[#173c31]"/><span><strong>I confirm this fitment applies to the exact vehicle configuration selected above.</strong><small className="mt-1 block leading-5 text-[#63706a]">Do not confirm a fitment based only on a similar model name or registration lookup.</small></span></label>
