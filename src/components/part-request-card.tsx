...
export function PartRequestCard({signedIn,filters,defaultText="",returnTo="/#marketplace"}:{signedIn:boolean;filters:MarketplaceFilters;defaultText?:string;returnTo?:string}){
 const safeReturnTo=safeInternalPath(returnTo,"/#marketplace");
 return <section className="mx-auto mt-6 max-w-2xl rounded-3xl border border-black/10 bg-[#f8f7f2] p-5 text-left">
  <div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#173c31] text-[#d4f44d]"><SearchX size={18}/></span><div><h4 className="font-black">Can&apos;t find the exact part?</h4><p className="mt-1 text-sm leading-6 text-[#63706a]">Send a privacy-safe request with your selected vehicle. SecondPart routes it to sellers whose inventory, donor vehicles or fitment evidence are relevant — and keeps matching as stock changes.</p></div></div>
  {signedIn?<form action={createPartRequest} className="mt-4 grid gap-3">
   ...
   <button className="rounded-xl bg-[#173c31] px-5 py-3 text-sm font-black text-white">Find this part for me</button>
  </form>:<div className="mt-4"><Link href={`/account?returnTo=${encodeURIComponent(safeReturnTo)}`} className="inline-block rounded-xl bg-[#173c31] px-5 py-3 text-sm font-black text-white">Sign in to find this part</Link></div>}
 </section>;
}
