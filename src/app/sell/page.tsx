...
import { Boxes,Rocket,ShieldCheck,Upload } from "lucide-react";
...
export default async function SellPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <><Header/><main className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
  ...
  <section className="mt-8 max-w-3xl rounded-3xl bg-[#173c31] p-6 text-white"><div className="flex items-start gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-white/10 text-[#d4f44d]"><Rocket size={20}/></span><div><p className="text-xs font-black uppercase tracking-[.16em] text-[#d4f44d]">For established automotive businesses</p><h2 className="mt-2 text-2xl font-black">Founding Seller Programme</h2><p className="mt-2 text-sm leading-6 text-white/70">Breakers, ATFs, garages and parts businesses with existing inventory can apply for the structured onboarding route, including bulk inventory workflows.</p><Link href="/founding-sellers" className="mt-4 inline-block rounded-xl bg-[#d4f44d] px-4 py-2.5 text-sm font-black text-[#173c31]">Explore Founding Seller onboarding</Link></div></div></section>
