...
export default async function SellPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <><Header/><main className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
  <p className="text-xs font-black uppercase tracking-[.2em] text-[#287154]">For garages and specialists</p>
  <h1 className="mt-3 max-w-3xl text-4xl font-black tracking-[-.045em] sm:text-5xl">Manage real inventory, fitment and product images.</h1>
  <p className="mt-4 max-w-2xl text-lg text-[#63706a]">Seller accounts can publish parts, maintain stock and specify exact vehicle compatibility.</p>
  ...
  {buyer
   ...
   :<Link href="/account?mode=signup&role=seller&returnTo=/dashboard" className="mt-8 inline-flex rounded-full bg-[#d4f44d] px-6 py-3.5 font-black">Create a seller account</Link>}
 </main></>;
}
