...
export default async function ModerationPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <><Header/><main className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
  <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#287154]">Admin</p><h1 className="mt-3 text-4xl font-black tracking-[-.045em]">Marketplace moderation</h1><p className="mt-2 max-w-2xl text-[#63706a]">Launch moderation for seller verification, listing reports and support.</p></div><div className="flex flex-wrap gap-2"><Link href="/admin/founding-sellers" className="inline-flex w-fit items-center gap-2 rounded-full border border-black/15 px-5 py-3 text-sm font-black"><Rocket size={17}/>Founding Sellers</Link><Link href="/admin/commerce" className="inline-flex w-fit items-center gap-2 rounded-full bg-[#173c31] px-5 py-3 text-sm font-black text-white"><Scale size={17}/>Commerce cases</Link><Link href="/admin/system" className="inline-flex w-fit items-center gap-2 rounded-full border border-black/15 px-5 py-3 text-sm font-black"><Settings2 size={17}/>System readiness</Link></div></div>
