...
export default async function ModerationPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const [verificationResult,reportsResult,supportResult,garageResult]=await Promise.all([
  ...
  supabase.from("marketplace_reports").select("id,reason,details,status,created_at,reported_profile_id,parts(id,title,slug),sellers(id,business_name,slug)",{count:"exact"}).eq("status","open").order("created_at").range((reportsPage-1)*pageSize,(reportsPage-1)*pageSize+pageSize),
 ...
 const reportedProfileIds=[...new Set(reportsRaw.map(row=>row.reported_profile_id).filter((id):id is string=>Boolean(id)))];
 const {data:reportedProfileRows}=reportedProfileIds.length
  ?await supabase.from("profiles").select("id,display_name,handle").in("id",reportedProfileIds)
  :{data:[] as Array<{id:string;display_name:string;handle:string}>};
 const reportedProfiles=new Map((reportedProfileRows??[]).map(profile=>[profile.id,profile] as const));
