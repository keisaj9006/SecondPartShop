...
import { addSellerProspectActivity,updateSellerProspect } from "./actions";
...
export default async function SellerProspectsPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <><Header/><main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14">
  ...
  {items.length?<div className="mt-6 grid gap-4">{items.map(item=>{const history=activitiesByProspect.get(item.id)??[];return <article key={item.id} className="rounded-3xl border border-black/10 bg-white p-5 sm:p-6"><div className="flex flex-col justify-between gap-5 lg:flex-row">
   ...
   <div className="mt-5 grid gap-4 border-t border-black/8 pt-5 lg:grid-cols-[1fr_1fr]">
    <section><div className="flex items-center gap-2"><History size={15} className="text-[#287154]"/><h3 className="text-sm font-black">Recent activity</h3></div>{history.length?<div className="mt-3 grid gap-2">{history.map(activity=>{const profile=Array.isArray(activity.profiles)?activity.profiles[0]:activity.profiles;return <div key={activity.id} className="rounded-xl bg-[#f8f7f2] p-3 text-xs"><div className="flex flex-wrap items-center justify-between gap-2"><strong className="capitalize">{activity.activity_type.replaceAll("_"," ")}</strong><span className="text-[#8a918e]">{new Date(activity.created_at).toLocaleString("en-GB")}</span></div>{activity.outcome&&<p className="mt-1 font-bold">{activity.outcome}</p>}{activity.note&&<p className="mt-1 leading-5 text-[#63706a]">{activity.note}</p>}<p className="mt-1 text-[10px] text-[#8a918e]">{profile?.display_name??"Admin"}{activity.next_action_at?" · next "+new Date(activity.next_action_at).toLocaleString("en-GB"):""}</p></div>;})}</div>:<p className="mt-3 text-xs text-[#63706a]">No activity logged yet.</p>}</section>
