...
export default async function GaragesPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const pageSize=24;
 const result=await getGaragePartnersPage((page-1)*pageSize,pageSize).catch(()=>({items:[],hasMore:false,offset:0,limit:pageSize}));
 return <><Header/><main className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
  ...
  {result.items.length?<><div className="mt-8 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{result.items.map(item=><article key={item.id} className="rounded-3xl border border-black/10 bg-white p-6">
   ...
   <p className="mt-2 flex items-center gap-2 text-sm font-bold"><MapPin size={15}/>{item.location} · {item.postcode}</p>
  ...
  </article>)}</div>{(page>1||result.hasMore)&&<nav className="mt-10 flex justify-center gap-3">{page>1&&<Link href={page===2?"/garages":"/garages?page="+(page-1)} className="rounded-full border border-black/15 px-5 py-3 text-sm font-black">Previous</Link>}<span className="px-3 py-3 text-sm font-bold text-[#63706a]">Page {page}</span>{result.hasMore&&<Link href={"/garages?page="+(page+1)} className="rounded-full bg-[#173c31] px-5 py-3 text-sm font-black text-white">Next</Link>}</nav>}</>:<div className="mt-8 rounded-3xl border border-dashed border-black/20 p-8 text-center"><h2 className="text-xl font-black">Garage partner recruitment is open</h2><p className="mt-2 text-sm text-[#63706a]">Approved workshops will appear here as the Buy + Fit network grows.</p></div>}
