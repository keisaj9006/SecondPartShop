...
import { getSavedPartIdsForParts } from "@/lib/data/marketplace";
...
export default async function RecentlyViewedPage(){
 ...
 const savedIds=listings.length?await getSavedPartIdsForParts(user.id,listings.map(item=>item.id)):[];
 return <><Header/><main className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
  ...
  {listings.length?<div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{listings.map(item=><ProductCard key={item.id} item={item} viewerId={user.id} saved={savedIds.includes(item.id)}/>)}</div>:<div className="mt-8 rounded-3xl border border-dashed border-black/20 bg-white px-6 py-16 text-center"><Clock3 className="mx-auto text-[#63706a]"/><h2 className="mt-4 text-xl font-black">No recently viewed parts yet</h2><Link href="/#marketplace" className="mt-3 inline-block font-black underline">Browse marketplace</Link></div>}
 </main></>;
}
