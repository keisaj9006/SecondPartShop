...
const contextKeys=["q","category","condition","min","max","vehicle","vr","cv","cy","cf","ce"] as const;
 ...
 return <><Header/><main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
  ...
  <div className="grid gap-8 lg:grid-cols-[1.05fr_.95fr]">
   ...
   <div>
    ...
    <div className="mt-6 grid gap-3 rounded-2xl bg-[#173c31] p-5 text-sm text-white">{item.seller.verified&&<span className="flex items-center gap-2"><ShieldCheck className="text-[#d4f44d]" size={18}/>Verified seller</span>}<span className="flex items-center gap-2"><MapPin className="text-[#d4f44d]" size={18}/>{item.seller.businessName}, {item.seller.location}</span><span className="flex items-center gap-2"><Truck className="text-[#d4f44d]" size={18}/>Stock and dispatch supplied by the seller</span></div>
   </div>
  </div>
 </main></>;
}
