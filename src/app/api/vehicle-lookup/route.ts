...
export async function POST(request:Request){
 let body:unknown;
 try{body=await request.json();}catch{return NextResponse.json({message:"Send a registration in JSON."},{status:400});}
 ...
 return NextResponse.json({registration:result.registration,vehicle:result.vehicle,vehicleId:result.vehicle.vehicleId??null},{status:200,headers:{"cache-control":"no-store"}});
}
