...
export async function PATCH(request:Request,{params}:{params:Promise<{partId:string}>}){
 ...
 try{
  ...
  const status=code==="photo_required"||code==="compatibility_evidence_required"?409:400;
  const known=[
   ...
   "duplicate_fitment","fitment_save_failed","photo_required","compatibility_evidence_required",
