...
export default async function AccountSecurityPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const [{data:pending},{data:policy}]=await Promise.all([
  supabase
   ...
   .select("id,requested_at,status,blocker_code,processing_started_at,attempt_count")
 ...
 return <><Header/><main className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
  ...
  <section className="mt-8 rounded-3xl border border-red-200 bg-red-50/50 p-5 sm:p-7">
   ...
   {pending?<div className="mt-5 rounded-2xl border border-red-200 bg-white p-4"><p className="font-black text-red-950">{pending.status==="processing"?"Account deletion is being processed":pending.status==="blocked"?"Account deletion is temporarily blocked":pending.status==="failed"?"Account deletion will be retried":"Deletion request pending"}</p><p className="mt-1 text-sm text-red-900/70">Requested {new Date(pending.requested_at).toLocaleDateString("en-GB")}.{pending.status==="blocked"?" We will retry automatically when active marketplace obligations are closed.":pending.status==="failed"?" A safe retry will run through maintenance.":""}</p>{(["requested","blocked"].includes(pending.status)&&pending.attempt_count===0)&&<form action={cancelAccountDeletion}><button className="mt-4 rounded-xl border border-red-300 px-4 py-2.5 text-sm font-black text-red-800">Cancel deletion request</button></form>}</div>:<AccountDeletionForm/>}
  </section>
 </main></>;
}
