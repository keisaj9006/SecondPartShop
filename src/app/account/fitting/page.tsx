...
export default async function BuyerFittingPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <><Header/><main className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
  ...
  {items.length?<div className="mt-7 grid gap-4">{items.map(item=><article key={item.id} className="rounded-3xl border border-black/10 bg-white p-5 sm:p-6">
   ...
   {["accepted","completed"].includes(item.status)&&<div className="mt-4 flex flex-wrap items-center gap-3"><Link href={"/fitting/"+item.id} className="rounded-xl bg-[#173c31] px-4 py-2.5 text-sm font-black text-white">{item.status==="accepted"?"Arrange fitting":"View fitting chat"}</Link><span className="text-xs font-bold text-[#63706a]">{item.status==="accepted"?"Use the private fitting chat to agree the appointment.":"Fitting completed · chat history retained."}</span></div>}
