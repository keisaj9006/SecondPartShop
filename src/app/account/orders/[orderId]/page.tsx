...
export default async function BuyerOrderDetailPage({params,searchParams}:{params:Promise<{orderId:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <><Header/><main className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-12">
  ...
  {order.paymentStatus==="processing"&&<div className="mt-5 rounded-2xl bg-amber-50 p-4 text-sm font-bold text-amber-900">Payment is processing. Do not start another checkout for this item; SecondPart will reconcile the Stripe result automatically.</div>}
  <div className="mt-7 grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
   <section className="grid gap-4">
    ...
    <div className="rounded-3xl bg-[#173c31] p-5 text-white"><p className="text-sm text-white/65">Order total</p><p className="mt-1 text-3xl font-black">{money(order.totalPence,order.currency)}</p>{["unpaid","requires_action"].includes(order.paymentStatus)&&<form action={resumeCheckout} className="mt-4"><input type="hidden" name="orderId" value={order.id}/><button className="rounded-xl bg-[#d4f44d] px-4 py-2.5 text-sm font-black text-[#173c31]">Resume secure checkout</button></form>}</div>
   </section>
