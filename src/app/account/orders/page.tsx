...
export default async function PurchasesPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <><Header/><main className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
  ...
  <h1 className="mt-2 text-3xl font-black tracking-[-.045em] sm:text-4xl">Purchases</h1>
  ...
  {orders.length?<div className="mt-8 grid gap-5">{orders.map(order=><article key={order.id} className="rounded-3xl border border-black/10 bg-white p-5 sm:p-6">
   <div className="flex flex-col justify-between gap-3 border-b border-black/8 pb-4 sm:flex-row sm:items-center">
    <div><p className="text-xs font-black uppercase tracking-wide text-[#287154]">Order {order.id.slice(0,8).toUpperCase()}</p><p className="mt-1 break-words text-sm text-[#63706a]">{new Intl.DateTimeFormat("en-GB",{dateStyle:"medium"}).format(new Date(order.createdAt))}</p><Link href={"/account/orders/"+order.id} className="mt-2 inline-block text-xs font-black text-[#287154] underline">View full order</Link></div>
   ...
   <div className="mt-4 grid gap-4">{order.items.map(item=><div key={item.id} className="rounded-2xl bg-[#f8f7f2] p-4">
    ...
    <div className="mt-3 flex flex-wrap gap-3 text-xs font-bold text-[#56625d]"><span className="inline-flex items-center gap-1"><PackageCheck size={14}/>{label(item.fulfilmentStatus)}</span>{item.trackingNumber&&<span className="inline-flex min-w-0 items-center gap-1 break-all"><Truck size={14} className="shrink-0"/>{item.trackingCarrier?`${item.trackingCarrier} · `:""}{item.trackingNumber}</span>}{item.fundsReleasedAt&&<Link href="/account/reviews" className="inline-flex items-center gap-1 text-[#287154] underline"><Star size={14}/>Leave verified review</Link>}{order.paymentStatus==="paid"&&<Link href={"/messages/"+item.id} className="font-black text-[#287154] underline">Message seller</Link>}{order.paymentStatus==="paid"&&["paid","preparing","ready_for_collection"].includes(item.fulfilmentStatus)&&<Link href={"/account/cases?item="+encodeURIComponent(item.id)+"&type=cancellation"} className="font-black text-amber-800 underline">Request cancellation</Link>}{order.paymentStatus==="paid"&&!["cancelled","refunded","returned","return_requested","dispute_open"].includes(item.fulfilmentStatus)&&<Link href={"/account/cases?item="+encodeURIComponent(item.id)} className="font-black text-amber-800 underline">Report problem / return</Link>}</div>
