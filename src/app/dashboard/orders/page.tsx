 ...
 return <><Header/><main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
  ...
  {sales.length?<div className="mt-8 overflow-hidden rounded-3xl border border-black/10 bg-white">
   ...
   {sales.map(sale=><div key={sale.orderItemId} className="grid gap-4 border-b border-black/8 p-5 last:border-0 md:grid-cols-[1.4fr_.8fr_.8fr_.8fr] md:items-center">
    <div><Link href={`/parts/${sale.partSlug}`} className="font-black hover:underline">{sale.partTitle}</Link><p className="mt-1 text-xs text-[#63706a]">Order {sale.orderId.slice(0,8).toUpperCase()} · {money(sale.unitPricePence*sale.quantity)} · Qty {sale.quantity}</p>{sale.trackingNumber&&<p className="mt-2 flex items-center gap-1 text-xs font-bold text-[#287154]"><Truck size={13}/>{sale.trackingCarrier?`${sale.trackingCarrier} · `:""}{sale.trackingNumber}</p>}</div>
