...
export default async function NewListingPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const [categories,donorPage,requestLeadPage]=await Promise.all([getCategories(),getDonorVehiclesPage(seller.id,{limit:50}),getSellerPartRequestLeads({limit:60})]);
 ...
 const requestedLead=requestLeadPage.items.find(lead=>lead.id===first(params.request));
