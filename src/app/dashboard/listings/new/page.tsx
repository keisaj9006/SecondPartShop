...
export default async function NewListingPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const [categories,donors]=await Promise.all([getCategories(),getDonorVehicles(seller.id)]);
 ...
 return <><Header/><main className="mx-auto max-w-4xl px-4 py-10 sm:px-6"><Link href="/dashboard" className="text-sm font-bold underline">Back to dashboard</Link><h1 className="mt-5 text-4xl font-black tracking-tight">Create listing</h1><p className="mt-2 text-[#63706a]">Add accurate identifiers, real product photos and only fitments you can support.</p><ListingForm categories={categories} donors={donors} defaultDonorId={defaultDonorId}/></main></>;
}
