...
import { getSellerPartRequestLead } from "@/lib/data/seller-request-leads";
...
export default async function NewListingPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const [categories,donorPage,requestedLead,sellerCheckoutReady]=await Promise.all([getCategories(),getDonorVehiclesPage(seller.id,{limit:20}),requestedRequest&&isUuid(requestedRequest)?getSellerPartRequestLead(requestedRequest).catch(()=>null):Promise.resolve(null),isSellerCheckoutReady(seller.id).catch(()=>false)]);
