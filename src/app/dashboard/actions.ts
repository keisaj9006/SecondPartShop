...
import type { ActionState,ListingActionState,ListingStatus,PartCondition,PartTestingStatus } from "@/lib/types";
...
export async function createSellerProfile(_previous:ActionState,formData:FormData):Promise<ActionState>{
 const {user}=await requireSeller("/dashboard");
 if(!await hasCurrentMarketplaceTerms(user.id))return {status:"error",message:"Accept the current Terms of Use and Privacy Policy in Account → Security before creating public marketplace content."};
 ...
 if(businessName.length<2||!location||description.length<20)return {status:"error",message:"Complete the seller name, location and a description of at least 20 characters."};
 if(!(["business","private"] as string[]).includes(sellerType))return {status:"error",message:"Choose a valid seller type."};
...
export async function updateSellerProfile(_previous:ActionState,formData:FormData):Promise<ActionState>{
 ...
 try{await persistSellerGeo(seller.id,user.id,sellerGeo);}
 catch{return {status:"error",message:"Seller profile was saved, but we could not prepare the postcode for distance search. Please save the profile again."};}
