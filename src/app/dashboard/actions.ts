...
export async function updateStock(formData:FormData){const {user}=await requireSeller("/dashboard");const seller=await getSellerForOwner(user.id);if(!seller)return;const partId=String(formData.get("partId")??"");const rawStock=Number(formData.get("stock"));if(!Number.isFinite(rawStock))return;const stock=Math.max(0,Math.floor(rawStock));const supabase=await createSupabaseServerClient();const {data:part}=await supabase.from("parts").select("status").eq("id",partId).eq("seller_id",seller.id).maybeSingle();if(!part||part.status==="reserved")return;const nextStatus=part.status==="active"&&stock===0?"sold":part.status==="sold"&&stock>0?"active":part.status;await supabase.from("parts").update({stock,status:nextStatus}).eq("id",partId).eq("seller_id",seller.id).neq("status","reserved");revalidatePath("/");revalidatePath("/dashboard");}
