   ...
   return <article key={lead.id} className="rounded-3xl border border-black/10 bg-white p-5 sm:p-6">
    <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
     <div className="min-w-0">
      <div className="flex flex-wrap gap-2">{lead.categoryName&&<span className="rounded-full bg-[#eef1eb] px-2.5 py-1 text-xs font-black">{lead.categoryName}</span>}<span className="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-black text-amber-900">Open request</span><span className="rounded-full bg-cyan-50 px-2.5 py-1 text-xs font-black text-cyan-900">Match {lead.matchScore}</span></div>
