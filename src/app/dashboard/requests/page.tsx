   ...
   return <article key={lead.id} className="rounded-3xl border border-black/10 bg-white p-5 sm:p-6">
    <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
     <div className="min-w-0">
      <div className="flex flex-wrap gap-2">{lead.categoryName&&<span className="rounded-full bg-[#eef1eb] px-2.5 py-1 text-xs font-black">{lead.categoryName}</span>}<span className="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-black text-amber-900">{lead.draftPartId?"Draft response":"Open request"}</span><span className="rounded-full bg-cyan-50 px-2.5 py-1 text-xs font-black text-cyan-900">{lead.matchScore>=100?"Strong match":lead.matchScore>=60?"Good match":"Relevant match"}</span></div>
     ...
     <div className="flex shrink-0 flex-col gap-2"><Link href={lead.draftPartId?"/dashboard/listings/"+lead.draftPartId+"/edit":"/dashboard/listings/new?"+params.toString()} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#173c31] px-4 py-3 text-sm font-black text-white">{lead.draftPartId?"Resume draft":"Create matching listing"}<ArrowUpRight size={16}/></Link><form action={dismissMatchedPartRequest}><input type="hidden" name="requestId" value={lead.id}/><button className="w-full rounded-xl border border-black/15 bg-white px-4 py-2.5 text-xs font-black text-[#56625d]">Not relevant · Dismiss</button></form></div>
    </div>
   </article>;
