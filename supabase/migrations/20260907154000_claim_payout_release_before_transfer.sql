...
declare
  item record;
begin
  select oi.id,oi.payout_status,oi.release_eligible_at,oi.funds_released_at,o.payment_status
  into item
...
create or replace function public.reset_order_item_payout_release_claim(p_order_item_id uuid)
returns boolean
language plpgsql
security definer
set search_path=''
...
begin
  ...
  where oi.id=p_order_item_id
    and oi.payout_status='releasing'
...
begin
  select oi.id,oi.order_id,oi.payout_status,oi.release_eligible_at,oi.funds_released_at,oi.provider_transfer_id,
         o.payment_status,o.buyer_id,p.title
  into item
  from public.order_items oi
  join public.orders o on o.id=oi.order_id
...
revoke all on function public.claim_order_item_payout_release(uuid) from public;
revoke execute on function public.claim_order_item_payout_release(uuid) from anon;
revoke execute on function public.claim_order_item_payout_release(uuid) from authenticated;
grant execute on function public.claim_order_item_payout_release(uuid) to service_role;

revoke all on function public.reset_order_item_payout_release_claim(uuid) from public;
revoke execute on function public.reset_order_item_payout_release_claim(uuid) from anon;
revoke execute on function public.reset_order_item_payout_release_claim(uuid) from authenticated;
grant execute on function public.reset_order_item_payout_release_claim(uuid) to service_role;
