-- Distance pagination V2.
-- Compute buyer-to-seller distance once per seller, not once per listing.
-- Return limit+1 candidates from the caller without COUNT(*) OVER(), so the
-- app can detect hasMore without forcing a full result count.

create or replace function public.marketplace_distance_page_v2(
...
as $$
  ...
  filtered as (
    select
      p.id as part_id,
      d.distance_miles,
      d.postcode_geocode_approximate,
      case when p_part_ids is null then null else array_position(p_part_ids,p.id) end as search_rank,
      p.created_at
    from public.parts p
    join seller_distances d on d.seller_id=p.seller_id
  ...
  order by
    ...
    f.created_at desc,
    f.part_id
  limit greatest(2,least(coalesce(p_limit,25),61))
  offset greatest(0,coalesce(p_offset,0));
$$;

revoke all on function public.marketplace_distance_page_v2(
  double precision,double precision,uuid[],uuid[],text,integer,integer,boolean,integer,integer
) from public;
grant execute on function public.marketplace_distance_page_v2(
  double precision,double precision,uuid[],uuid[],text,integer,integer,boolean,integer,integer
) to anon,authenticated;

create or replace function public.marketplace_catalogue_distance_page_v2(
  p_variant_id uuid,
  p_year smallint,
  p_buyer_lat double precision,
  p_buyer_lon double precision,
...
revoke all on function public.marketplace_catalogue_distance_page_v2(
...
grant execute on function public.marketplace_catalogue_distance_page_v2(
  uuid,smallint,double precision,double precision,text,integer,uuid[],uuid[],text,integer,integer,boolean,boolean,integer,integer
) to anon,authenticated;
