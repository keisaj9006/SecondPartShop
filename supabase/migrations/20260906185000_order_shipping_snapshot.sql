...
alter table public.orders
  drop constraint if exists orders_shipping_name_length;
alter table public.orders
  add constraint orders_shipping_name_length
  check (shipping_name is null or char_length(shipping_name)<=160);
...
revoke all on function public.confirm_checkout_paid(uuid,text,text,text,text,text,jsonb) from public;
